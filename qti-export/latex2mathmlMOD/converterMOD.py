...
from latex2mathmlMOD.aggregatorMOD import aggregate
from latex2mathmlMOD.commandsMOD import COMMANDS, MATRICES
from latex2mathmlMOD.symbols_parserMOD import convert_symbol
...
def convert(latex: str, xmlns: str = "http://www.w3.org/1998/Math/MathML") -> str:
    math = Element("m:math")
    # ***DELETED*** math.attrib["xmlns"] = xmlns
    row = SubElement(math, "m:mrow")
    # ***ADDED*** <m:mrow class="MJX-TeXAtom-ORD">
    row.attrib["class"] = "MJX-TeXAtom-ORD"
...
def _convert(tree: Element) -> str:
    indent(tree)
    return unescape(tostring(tree, encoding="unicode")) # , pretty_print=True
    ...
    for row in param:
        ...
        for i in iterable:
            ...
            if align:
                ...
                mtd = SubElement(mtr, "m:mtd", columnalign=column_align)
            else:
                mtd = SubElement(mtr, "m:mtd")
            if isinstance(element, list):
                _classify_subgroup(element, mtd)
            elif element in COMMANDS:
                _convert_command(element, row, i, iterable, mtd)
...
def _convert_command(
    ...
    for j in range(params):
        ...
        if element == r"\left" or element == r"\right":
            ...
            if isinstance(param, list):
                _parent = SubElement(new_parent, "m:mrow")
    ...
    if element in (r"\overline", r"\bar"):
        mo = SubElement(new_parent, "m:mo", stretchy="true")
        mo.text = "&#x000AF;"
...
def _classify(_element: str, parent: Element, is_math_mode: bool = False) -> None:
    ...
    if re.match(r"\d+(.\d+)?", _element):
        ...
        mo = SubElement(parent, "m:mo")
        mo.text = _element if symbol is None else symbol # "&#x{};".format(symbol)
    elif (
        symbol
        # and (
        ...
        mo = SubElement(parent, "m:mo" if is_math_mode else "m:mi")
        mo.text = symbol # "&#x{};".format(symbol)
    elif _element.startswith("\\"):
        tag = SubElement(parent, "m:mo" if is_math_mode else "m:mi")
        if symbol:
            tag.text = symbol # "&#x{};".format(symbol)
        else:
            tag.text = _element
    else:
        tag = SubElement(parent, "m:mo" if is_math_mode else "m:mi")
